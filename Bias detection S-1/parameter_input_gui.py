...
def update_record_in_db(record_id, bias_param, desc_bias_param):
    ...
    try:
        ...
        cursor.execute(f"""
            UPDATE {TABLE_NAME}
            SET bias_parameter = ?, description_bias_parameter = ?, timestamp = ?
            WHERE id = ?
        """, (bias_param, desc_bias_param, datetime.now(), record_id))
        conn.commit()
        return True
    except sqlite3.Error as e:
...
def check_if_parameters_exist_excluding_id(bias_param, desc_bias_param, exclude_id):
    """Checks if the given parameter combination already exists in the database, excluding a specific ID."""
    conn = None
    try:
        conn = sqlite3.connect(DB_ABSOLUTE_PATH)
        cursor = conn.cursor()
        cursor.execute(f"""
            SELECT 1 FROM {TABLE_NAME}
            WHERE bias_parameter = ? AND description_bias_parameter = ? AND id != ?
        """, (bias_param, desc_bias_param, exclude_id))
        return cursor.fetchone() is not None
    except sqlite3.Error as e:
        messagebox.showerror("Database Error", f"Failed to check parameters (excluding ID): {e}")
...
def on_delete_record_click(tree, refresh_func):
    """Handles the delete button click event in the viewer."""
    selected_items = tree.selection()
    if not selected_items:
        messagebox.showwarning("Selection Error", "Please select a record to delete.")
        return

    # Allow deleting multiple selected items
    confirm = messagebox.askyesno("Confirm Deletion", f"Are you sure you want to delete {len(selected_items)} selected record(s)?")

    if confirm:
        deleted_count = 0
        for item in selected_items:
            record_id = tree.item(item, "values")[0] # ID is the first column
            if delete_record_from_db(record_id):
                deleted_count += 1
            else:
                # Optionally break or continue based on desired behavior on failure
                pass

        if deleted_count > 0:
            messagebox.showinfo("Deletion Successful", f"Successfully deleted {deleted_count} record(s).")
            refresh_func() # Refresh the treeview after deletion
        elif len(selected_items) > 0:
             messagebox.showwarning("Deletion Failed", "No records were deleted.")

def on_edit_record_click(tree, refresh_func):
    """Handles the edit button click event in the viewer."""
    selected_items = tree.selection()
    if not selected_items:
        messagebox.showwarning("Selection Error", "Please select a record to edit.")
        return
    if len(selected_items) > 1:
        messagebox.showwarning("Selection Error", "Please select only one record to edit.")
        return

    selected_item = selected_items[0]
    record_values = tree.item(selected_item, "values")

    # Extract data: ID, Timestamp, Bias Parameter, Description
    record_id = record_values[0]
    current_bias_param = record_values[2]
    current_desc_bias_param = record_values[3]

    # Open a new window for editing
    edit_window = tk.Toplevel(root)
    edit_window.title(f"Edit Record ID: {record_id}")
    edit_window.transient(root) # Keep edit window on top of main window
    edit_window.grab_set() # Prevent interaction with other windows until edit is done

    edit_frame = ttk.Frame(edit_window, padding="10")
    edit_frame.pack(expand=True, fill=tk.BOTH)

    ttk.Label(edit_frame, text="Bias Parameter Code:").grid(row=0, column=0, sticky=tk.W, pady=5, padx=5)
    edit_bias_param_entry = ttk.Entry(edit_frame, width=40)
    edit_bias_param_entry.insert(0, current_bias_param)
    edit_bias_param_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=5, padx=5)

    ttk.Label(edit_frame, text="Description of Bias Parameter:").grid(row=1, column=0, sticky=tk.W, pady=5, padx=5)
    edit_desc_bias_param_entry = ttk.Entry(edit_frame, width=40)
    edit_desc_bias_param_entry.insert(0, current_desc_bias_param)
    edit_desc_bias_param_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5, padx=5)

    def on_save_changes():
        new_bias_param = edit_bias_param_entry.get().strip()
        new_desc_bias_param = edit_desc_bias_param_entry.get().strip()

        # Validation and Duplicate Check (excluding the current record's ID)
        if not new_bias_param or not new_desc_bias_param:
             messagebox.showerror("Input Error", "Fields cannot be empty.")
             return
        if check_if_parameters_exist_excluding_id(new_bias_param, new_desc_bias_param, record_id):
             messagebox.showerror("Input Error", "This parameter combination already exists for another record.")
             return

        if update_record_in_db(record_id, new_bias_param, new_desc_bias_param):
            messagebox.showinfo("Success", "Record updated successfully!")
            edit_window.destroy() # Close the edit window
            refresh_func() # Refresh the main viewer treeview
        # Error message is handled within update_record_in_db

    save_changes_button = ttk.Button(edit_frame, text="Save Changes", command=on_save_changes)
    save_changes_button.grid(row=2, column=0, columnspan=2, pady=10)
...
def open_database_viewer_window():
    ...
    button_frame = ttk.Frame(viewer_frame)
    ...
    refresh_button.grid(row=3, column=0, pady=5, sticky='ew') # Moved below button_frame

    edit_button = ttk.Button(button_frame, text="Edit Selected Record", command=lambda: on_edit_record_click(tree, refresh_treeview_data))
    edit_button.pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)

    delete_button = ttk.Button(button_frame, text="Delete Selected Record(s)", command=lambda: on_delete_record_click(tree, refresh_treeview_data))
